...
test("phone number with less than 10 digits", () => {
  ...
  userEvent.click(screen.getByRole("button", { name: /continue/i }));
  expect(
    screen.getByText(/Phone number must consist of 10 digits/)
  ).toBeTruthy();
});
...
test("land line phone number", () => {
  ...
  userEvent.click(screen.getByRole("button", { name: /continue/i }));
  expect(
    screen.getByText(/Phone number should be a mobile number. Not a land line/)
  ).toBeTruthy();
});
...
test("invalid service provider", () => {
  ...
  expect(
    screen.getByText(
      /This number is not valid. It must be 070, 071, 072, 075, 076, 077, or 078/
    )
  ).toBeTruthy();
});
...
test("password with less than 8 charaters", () => {
  ...
  userEvent.click(screen.getByRole("button", { name: /continue/i }));
  expect(
    screen.getByText(/Password needs to be 8 characters or more/)
  ).toBeTruthy();
});
