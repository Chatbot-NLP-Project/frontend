...
class ActionProvider {
  ...
  sendFeedback = (message) => {
    ...
    Axios.post("https://xyronchatbot.azurewebsites.net/sendFeedback", {
      feedback: feedback,
    }).then((response) => {
  ...
  predictHandler = (message) => {
    var msg;
    Axios.post("https://xyronhealthcarebot.azurewebsites.net/predict", { diseases: message }).then(
      (response) => {
        console.log(message);
...
=======
    Axios.post("https://xyronhealthcarebot.azurewebsites.net/getDoc", {
      ...
      // this.setChatbotMessage(msg);
      if (response.data.er == 1) {
        msg = this.createChatBotMessage("Please re-enter the speciality");
        this.setChatbotMessage(msg);
      } else {
        console.log(response.data.doc);
        this.setState((state) => ({ ...state, doctors: response.data.doc }));
